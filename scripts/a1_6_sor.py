...
Solves the steady-state diffusion equation with SOR iteration
...
print(f"SOR (omega={omega}): converged={result.converged}, "
...
ax.set_title(rf"SOR solution c(x, y), $\omega$={omega}")
ax.set_xlabel("x")
ax.set_ylabel("y")
ax.set_aspect("equal")
...
ax = axes[1]
mid_i = N // 2
ax.plot(grid.y, result.y[mid_i, :], "o-", markersize=3, label=f"SOR ({omega})")
